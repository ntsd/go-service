	...
	tests := []testData{
		{
			...
			request: func() *http.Request {
				req := httptest.NewRequest("PUT", "/v1/users/foo", nil)
			...
			mockStorage: func(ctrl *gomock.Controller) storage.Storage {
				...
				mockStorage.EXPECT().UpdateUserByID(gomock.Any(), "foo").Return(nil)
				return mockStorage
			},
		...
		},
		{
			...
			mockStorage: func(ctrl *gomock.Controller) storage.Storage {
				mockStorage := mock_storage.NewMockStorage(ctrl)
				mockStorage.EXPECT().UpdateUserByID(gomock.Any(), "foo").Return(errors.New("error"))
				return mockStorage
			},
		...
		},
		{
			...
			mockStorage: func(ctrl *gomock.Controller) storage.Storage {
				mockStorage := mock_storage.NewMockStorage(ctrl)
				mockStorage.EXPECT().UpdateUserByID(gomock.Any(), "foo").Return(&pgconn.PgError{Code: "1"})
				return mockStorage
			},
		...
		},
		{
			...
			mockStorage: func(ctrl *gomock.Controller) storage.Storage {
				mockStorage := mock_storage.NewMockStorage(ctrl)
				mockStorage.EXPECT().UpdateUserByID(gomock.Any(), "foo").Return(&pgconn.PgError{Code: pgerrcode.UniqueViolation})
				return mockStorage
			},
			wantStatus: fiber.StatusUnprocessableEntity,
		},
		{
			...
			mockStorage: func(ctrl *gomock.Controller) storage.Storage {
				mockStorage := mock_storage.NewMockStorage(ctrl)
				mockStorage.EXPECT().UpdateUserByID(gomock.Any(), "foo").Return(gorm.ErrRecordNotFound)
				return mockStorage
			},
